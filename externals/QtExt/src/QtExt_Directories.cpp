...
bool Directories::copyDirRecursively(const QString &srcPath, const QString &dstPath) {
	QDir srcDir(srcPath);
	if (!srcDir.exists())
		return false;
	...
	QDir dstDir(dstPath);
	if (!dstDir.exists()) {
		if (!dstDir.mkpath("."))   // create destination root
			return false;
	}
	...
	for (const QFileInfo &entry : qAsConst(entries)) {
		...
		QString dstItemPath = dstDir.filePath(entry.fileName());
		...
		if (entry.isDir()) {
			...
			if (!QFile::copy(srcItemPath, dstItemPath)) {
				qCritical() << "Failed copying:" << srcItemPath;
				return false;
			}
		}
	}
