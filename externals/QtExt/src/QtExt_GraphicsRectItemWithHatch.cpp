...
namespace QtExt {
	...
	void GraphicsRectItemWithHatch::setRotated(bool rotated) {
		m_rotated = rotated;
	}
										  ...
										  QWidget *widget)
	{
		...
		// draw fill without border first
		QPen savedPen = pen();
		...
		if (m_hatchingType != HT_NoHatch) {
			...
			switch(m_hatchingType) {
				...
				case HT_InsulationHatch: {
					drawInsulationHatch(painter, rect(), m_distance, m_rotated);
					break;
				}
				default: break;
			}
			...
			// draw only left/right border lines on top of hatching
			painter->save();
