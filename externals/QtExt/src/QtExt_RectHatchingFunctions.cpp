...
void drawInsulationHatch(QPainter* painter, const QRectF& rect, qreal density, bool forceVertical) {
	Q_ASSERT(painter);
	...
	if( height >= width || forceVertical) {
		int n = std::max(1, 2 + int(10.0 * height / (width * density)));
		qreal hs = height / n;
		qreal r = hs / 2.0;
